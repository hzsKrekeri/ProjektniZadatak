    ...
    for t in result_set:
        ...
        for m in members:
            created_member = TeamMember(id=m[0], first_name=m[1], last_name=m[2], email=m[3], phone_number=m[4],
                                        school=m[5], city=m[6], team=created_team.id)
...
def get_team_member(team_member_id):
    ...
    conn = _connect()  # todo use connection as context manager
    c = conn.cursor()
    query = """SELECT id,first_name,last_name,email,phone_number,school,city,team_id FROM team_member where id=?"""
    ...
    for t in result_set:
        created_member = TeamMember(id=t[0], first_name=t[1],last_name=t[2], email=t[3], phone_number=t[4], school=t[5],city=t[6],team=t[7])

        member.append(created_member)
...
def create_team(data):
    print("NOERRORS1______________________________________")
    conn = _connect()  # todo use connection as context manager
    c = conn.cursor()
    ...
    print("NOERRORS2______________________________________ ")
...
def delete_team_member(team_member_id):
    ...
    status = c.execute(delete_query,(team_member_id,))
    success = False
...
def add_team_member(data):
    ...
    insert_query = """INSERT INTO team_member (first_name, last_name, email, phone_number, school, city, team_id) 
        VALUES (?,?,?,?,?,?,?)"""
    c.execute(insert_query,(data['first_name'],data['last_name'],data['email'],data['phone_number'],data['school'],data['team_id']))
    data['id']=c.lastrowid
    conn.commit()
    c.close()
    conn.close()
